...
from ovh_voip_spam_filter.patterns import BlockPattern

_METROPOLE_PREFIXES = [
    "162",
    "163",  # operators (réseaux des démarcheurs niveau 1)
    "270",
    "271",
    "377",
    "378",
...
def arcep_fallback_patterns() -> list[BlockPattern]:
    return [
        *(
            BlockPattern(name="Préfixe démarchage ARCEP (métropole)", raw=f"33{p}")
            for p in _METROPOLE_PREFIXES
        ),
        *(
            BlockPattern(name="Préfixe démarchage ARCEP (outre-mer)", raw=f"33{p}")
            for p in _OUTRE_MER_PREFIXES
        ),
        *(
            BlockPattern(name="Préfixe démarchage ARCEP (OnOff)", raw=f"33{p}")
