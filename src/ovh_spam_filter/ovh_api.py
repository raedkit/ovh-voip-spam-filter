...
class OvhClient:
            ...
            try:
                ...
                if exc.code == 429 and attempt < self.max_retries_on_429:
                    ...
                    logger.warning(
                        ...
                        method, path, sleep_for, attempt, self.max_retries_on_429,
    ...
    def _compute_429_sleep(self, retry_after_header: str | None, backoff: float) -> float:
        ...
        return min(backoff * jitter, BACKOFF_CAP_SECONDS)
    ...
    def _maybe_adapt_throttle(self) -> None:
        if self._consecutive_429 >= AUTO_ADAPT_429_THRESHOLD and self.min_interval_seconds > 0:
            ...
            logger.warning(
                ...
                AUTO_ADAPT_429_THRESHOLD, old, self.min_interval_seconds,
            )
    ...
    def _headers(self, method: str, url: str, body_str: str, signed: bool) -> dict[str, str]:
        headers = {
            ...
            "User-Agent": "ovh-voip-spam-filter/0.1",
        }
        if not signed:
            return headers
    ...
    def get_screen_list_entry(
        ...
        return self.get(
            f"/telephony/{billing_account}/screen/{service_name}/screenLists/{entry_id}"
        ) or {}
