...
from ovh_spam_filter.logging_setup import get as _get_logger
from ovh_spam_filter.ovh_api import OvhApiError, OvhClient
...
def load_target() -> TargetState:
    ...
    try:
        ...
        logger.info(
            ...
            snap.version, snap.total_patterns, len(patterns),
        )
        ...
        logger.warning(
            "Saracroche unreachable (%s) — degraded mode with hard-coded ARCEP", exc,
        )
        return TargetState(
...
def compute_plan(
    ...
    current_blacklist = [
        e for e in current if e.type == "incomingBlackList"
    ]
...
def ensure_blacklist_enabled(
    client: OvhClient, billing_account: str, service_name: str
) -> None:
    ...
    logger.warning(
        ...
        "(preserving outgoing=%s)", incoming, outgoing, outgoing,
    )
    client.set_screen_state(
        billing_account, service_name,
        incoming="blacklist", outgoing=outgoing,
    )
...
def apply_plan(
    ...
    for call_number in plan.to_add:
        try:
            client.add_screen_list_entry(
                billing_account, service_name, call_number=call_number,
            )
            result.added += 1
            logger.info("Added %s", call_number)
        except OvhApiError as exc:
            result.failed += 1
            result.errors.append(f"POST {call_number}: {exc}")
            logger.error("POST %s failed: %s", call_number, exc)

    for entry in plan.to_remove:
        try:
            client.delete_screen_list_entry(billing_account, service_name, entry.id)
            result.removed += 1
            logger.info("Removed id=%d callNumber=%s", entry.id, entry.call_number)
        except OvhApiError as exc:
            result.failed += 1
            result.errors.append(f"DELETE id={entry.id}: {exc}")
            logger.error("DELETE id=%d failed: %s", entry.id, exc)
