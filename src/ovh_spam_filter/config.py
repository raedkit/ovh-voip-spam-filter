...
from ovh_spam_filter.ovh_api import ENDPOINTS, OvhCredentials
...
def load(config_path: Path | None = None) -> AppConfig:
    ...
    if endpoint not in ENDPOINTS:
        raise ConfigError(f"Unknown OVH_ENDPOINT {endpoint!r} (expected one of {sorted(ENDPOINTS)})")
    ...
    rate_limit_ms = _int_env("RATE_LIMIT_MS") or file_data.get("rate_limit_ms") or DEFAULT_RATE_LIMIT_MS
